...
import { Layers, Database, Shield, FileText, Briefcase, HelpCircle, ChevronRight, Filter, Check, ChevronDown } from 'lucide-react';
...
export default function VoiceAgent() {
    ...
    // Check Qdrant status periodically
    useEffect(() => {
        const checkStatus = async () => {
            try {
                const res = await fetch(`http://${window.location.hostname}:3001/health`);
                const data = await res.json();
                setIsQdrantConnected(data.qdrantConnected);
            } catch (e) {
    ...
    // Sync category with backend when connected or changed
    useEffect(() => {
        if (status === 'connected') {
        ...
        <div className="relative w-full h-full flex overflow-hidden bg-slate-950">
            ...
            <div className="flex-1 flex flex-col relative overflow-hidden">
                ...
                <header className="relative z-50 flex items-center justify-between px-8 py-6">
                    {/* Filter Dropdown in Top Left */}
                    <div className="flex items-center gap-4">
                        <div className="relative" ref={filterRef}>
                            <button
                                onClick={() => setIsFilterOpen(!isFilterOpen)}
                                className={`
                                    flex items-center gap-3 px-5 py-2.5 rounded-2xl border backdrop-blur-xl transition-all duration-300
                                    ${isFilterOpen 
